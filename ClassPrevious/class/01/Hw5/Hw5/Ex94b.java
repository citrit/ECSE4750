...
public class Ex94b extends Frame  {
    ...
    vtkMSPanel panel = null;
    
    public static void main( String args[] ) {

	Ex94b f = new Ex94b();

        f.panel = new vtkMSPanel();
	
	vtkPLOT3DReader r = new vtkPLOT3DReader();
	r.SetXYZFileName("../combxyz.bin");
	r.SetQFileName("../combq.bin");
	...
	if (false) {
		...
		sl  = new vtkStreamLine();
		sl.SetInput(r.GetOutput());
		sl.SetSource(points.GetOutput());
		sl.SetMaximumPropagationTime(100);
		sl.SetIntegrationStepLength(0.2F);
		sl.SetStepLength(0.02);
		sl.SpeedScalarsOn();
		sl.SetTerminalSpeed(.1);
		sl.Update();
	}
	else {
		vtkProbeFilter probe = new vtkProbeFilter();
		probe.SetInput(r.GetOutput());
		probe.SetSource(append.GetOutput());
		...
		sl.SpeedScalarsOn();
		sl.SetTerminalSpeed(0.1);
	}
	
	vtkPolyDataMapper cMapper = new vtkPolyDataMapper();
	cMapper.SetInput(sl.GetOutput());
