...
//  	double[] range = r.GetOutput().GetScalarRange();
//  	vtkContourFilter c = new vtkContourFilter();
//  	c.SetInput(probe.GetOutput());
	//  	c.GenerateValues(50, (float)range[0], (float)range[1]);

	vtkHedgeHog hog = new vtkHedgeHog();
	hog.SetInput(probe.GetOutput());
	System.out.println("scale " + hog.GetScaleFactor());
	hog.SetScaleFactor(.009);

	vtkPolyDataMapper cMapper = new vtkPolyDataMapper();
	cMapper.SetInput(hog.GetOutput());
