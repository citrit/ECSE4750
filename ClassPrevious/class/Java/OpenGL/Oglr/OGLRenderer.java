				...
				implements KeyListener, Renderer, ComponentListener, MouseMotionListener,MouseListener
{
	...
	JoglCanvas gl;
	/** SceneClock initialized if there is an action added */
	protected SceneClock SClock;
	...
	public void beginDL(int dlnum)
	{
		...
		//}
		gl.newList(dlnum, GL.COMPILE_AND_EXECUTE);
	}
	...
	{
		if (this.SClock == null) {
			SClock = new SceneClock(this);
		}
		SClock.addAction(act);
	}											  

	/** Start the scene clock, does nothing if no actions were added */
	public void startScene()
	{
		if (SClock != null && ClockThread == null) {
			ClockThread = new Thread(SClock);
			ClockThread.start();
		}
	}
