...
main(int argc, char *argv[]) {
	...
	colors[0] = 0.8;colors[1] = 0;colors[2] = 0;colors[3] = 1;
	for (i=1;i<=rCnt;i++) {
		...
		case MFAL_REGION:
			...
			colors[0] = float(rand())/float(RAND_MAX);colors[1] = float(rand())/float(RAND_MAX);
			colors[2] = float(rand())/float(RAND_MAX);colors[3] = 1;
			*mtSet += new Material(colors);
			// REad in points
