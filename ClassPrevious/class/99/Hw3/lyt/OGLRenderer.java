...
public class OGLRenderer extends JoglCanvas implements KeyListener, Renderer
{
	...
	{
		for(int i=1; i<=MAX_NUM_DL; i++)
		{
			if(id[i]==false) return i;
		}
		return -1;
	}
	...
	public void render()
	{
		...
		for (int i=0;i<ActorSet.size();i++) {
			// If the actor is newer than the render, render it
//			if(getTime()<((Actor)ActorSet.elementAt(i)).getTime())
...
//	public void setSize(int w, int h) { gl.setSize(w,h); }
	...
	public void setPointSize(float f) { gl.pointSize(f); }

	public long getTime() { return objectTime; }
	public void updateTime() { objectTime = System.currentTimeMillis(); }
}
