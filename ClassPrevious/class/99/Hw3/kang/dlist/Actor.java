...
public class Actor extends ParentObject
{
	...
	public void render(Renderer aren)
	{
		...
		{
			if(DLs.indexOf(new Integer(i+1)) >= 0)
			{
				DLs.removeElement(new Integer(i+1));
				aren.beginDL(i+1);
				((Cell)cells.elementAt(i)).render(aren);
				aren.endDL();
				System.out.println("Generated dlist...");
			}
			else {
				System.out.println("Using dlist...");
				aren.callDL(i+1);
			}
		}
		aren.popMatrix();
	...
	public void generateDL()
	{
		DL++;
		DLs.addElement(new Integer(DL));
	}
