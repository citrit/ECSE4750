...
StructuredGrid::DrawColoredGrid(Renderer *aren) {
  ...
  for (int i = 0;i < DimY-1; i++) {
	...
	aren->BeginDraw(Renderer::LINE_LOOP);
	   ...
	   _pt = GetPoint(cube);_pt.z = (*Data)[cube];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+1]);
	   _pt = GetPoint(cube+1);_pt.z = (*Data)[cube+1];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+DimX+1]);
	   _pt = GetPoint(cube+DimX+1);_pt.z = (*Data)[cube+DimX+1];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+DimX]);
	   _pt = GetPoint(cube+DimX);_pt.z = (*Data)[cube+DimX];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	aren->EndDraw();
      }
