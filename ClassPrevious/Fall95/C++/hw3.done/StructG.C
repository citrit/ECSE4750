...
StructuredGrid::DrawColoredGrid(Renderer *aren) {
  ...
  for (int i = 0;i < DimY-1; i++) {
	...
	aren->BeginDraw(Renderer::LINE_LOOP);
	   mat = Data->LookupValue((*Data)[cube]);
	   _pt = GetPoint(cube);if (ScalarIsZ) _pt.z = (*Data)[cube];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+1]);
	   _pt = GetPoint(cube+1);if (ScalarIsZ) _pt.z = (*Data)[cube+1];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+DimX+1]);
	   _pt = GetPoint(cube+DimX+1);if (ScalarIsZ) 
	                                 _pt.z = (*Data)[cube+DimX+1];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	   mat = Data->LookupValue((*Data)[cube+DimX]);
	   _pt = GetPoint(cube+DimX);if (ScalarIsZ) _pt.z = (*Data)[cube+DimX];
	   aren->SetMaterial(mat);
	   aren->Vertex(_pt.x, _pt.y, _pt.z); 
	aren->EndDraw();
      }
